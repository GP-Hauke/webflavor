...
function loadXMLData() {
  if(settingsLoaded === false) {
    GetInterfaceXML("resources/xml/settings.xml");
    return;
  }
  ...
  if(courseData.HAS_RESOURCES === 'true' && resourcesLoaded === false) {
    GetInterfaceXML("resources/xml/course_resources.xml");
    return;
  }

  if(courseData.MENU_PLACEMENT !== 'none' && navigationLoaded === false) {
    GetInterfaceXML("resources/xml/navigation.xml");
    return;
  }

  if(courseData.cardData !== undefined && cardContentLoaded === false) {
    GetInterfaceXML("resources/xml/card_content.xml");
    return;
  }
  ...
  if(courseData.HAS_STRINGS === 'true' && stringsLoaded === false) {
    GetInterfaceXML("resources/xml/strings.xml");
    return;

  }

  if(courseData.HAS_GLOSSARY === 'true' && glossaryLoaded === false) {
    GetInterfaceXML("resources/xml/glossary.xml");
    return;
...
function buildTopNav() {
  ...
  var navMarkup = '';
  $.get("resources/components/ui/nav.html", function(data) {
    navMarkup = data;
...
function buildLeftNav() {
  ...
  var navMarkup = '';
  $.get("resources/components/ui/navLeft.html", function(data) {
    navMarkup = data;
    $("#navContainer").append(navMarkup);
...
function loadPage() {
  ...
  document.getElementById("contentFrame").src="content/"+courseData.chapters[currentChapter].pages[currentPage].file+".html";
...
function getFooterNav() {
  ...
  var footerNavHTML = '<span><img src="'+courseData.THEME_PATH+'/media/img/btn_play_pause_inactive.png" alt="play/pause button"></span>';
  ...
  if(currentPage != courseData.chapters[currentChapter].pages.length-1 || currentChapter != courseData.chapters.length-1) {
    ...
    if(hasLocks) {
      nextBtnImgPath = '/media/img/btn_next.png';
    } else {
      nextBtnImgPath = '/media/img/btn_next.png';
    }

    footerNavHTML = footerNavHTML + '<a class="next" href="#" onclick="nextPage();"><img src="'+courseData.THEME_PATH+''+nextBtnImgPath+'" alt="go to next page"></a>';

  } else {
    nextBtnImgPath = '/media/img/btn_next_inactive.png';
  ...
  if(currentPage != 0 || currentChapter != 0) {
    backBtnImgPath = '/media/img/btn_back.png';

    footerNavHTML = '<a class="back" href="#" onclick="prevPage();"><img src="'+courseData.THEME_PATH+''+backBtnImgPath+'" alt="go to previous page"></a>' + footerNavHTML;

  } else {
    backBtnImgPath = '/media/img/btn_back_inactive.png';
